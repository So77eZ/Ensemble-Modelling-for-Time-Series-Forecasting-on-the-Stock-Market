# Архитектура проекта: Stock Price Forecasting Model v16
...
[Бэктест] → сравнение с реальными данными → accuracy report (.txt)
           включает: Error%, Naive Error%, R² on returns, IC (Spearman),
...
| --- | --- | --- |
| RMSE | Walk-forward | Root Mean Squared Error (среднеквадратичная ошибка) |
| MAE | Walk-forward | Mean Absolute Error (средняя абсолютная ошибка) |
| R² | Walk-forward | Коэффициент детерминации на ценах (высокий из-за трендовости ряда — не является основной мерой качества) |
| Error % | Бэктест | Ошибка прогноза цены в % от реальной цены |
| Naive Error % | Бэктест | Ошибка тривиального baseline (цена не изменится); показывает, превосходит ли модель простейший прогноз |
| R² on returns | Бэктест | R² по предсказанным и фактическим доходностям (% изменения цены); честная мера качества, не зависящая от тренда |
| IC (Spearman) | Бэктест | Information Coefficient — ранговая корреляция предсказанных и реальных доходностей; стандарт quant-индустрии (IC > 0.05 значимо) |
| CI Coverage | Бэктест | Доля случаев, когда реальная цена попала в доверительный интервал |
| Direction Accuracy | Бэктест | Доля горизонтов, где модель верно предсказала направление движения (вверх/вниз) |
