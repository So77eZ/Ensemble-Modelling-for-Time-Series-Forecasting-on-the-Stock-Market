...
- **Disjoint feature set для XGBoost (комплементарность с LSTM)** — v15.11 урезал LSTM до 7 фич (`_LSTM_FEATURES` = OHLCV + Price_Change_1 + Vol_Return_10), но XGB по-прежнему получает **полный** набор 46 фич, **включая** те же 7. Эксперимент: дать XGB **disjoint** подмножество (без `_LSTM_FEATURES`) → 39 фич (technicals SMA/EMA/RSI/MACD/BB/ATR/Stoch/ADX/Momentum + Price_Change_5 + Vol_Return_5/20 + Return_MA_5/10 + relative + macro + дивы + сезонные + fundamental). Все technicals — производные от OHLCV, информация сохраняется. Гипотеза: усилит комплементарность LSTM/XGB → Ridge даст LSTM ненулевой вес чаще (сейчас ~6/15 кейсов). Trade-off: XGB потеряет raw price для абсолютной калибровки → возможен рост MAE. Реализация: ~5 строк в `prepare_and_train_model` (фильтр `xgb_features = [f for f in features if f not in _LSTM_FEATURES]`). Прогон: SBER/GAZP/LKOH, multi-date backtest 12 дат × 3 горизонта × 2 конфигурации.
...
- **Fallback на локальный OHLCV-кэш при полной недоступности MOEX** — сейчас если все три уровня загрузки свечей упадут (`moexalgo` retry × 3 → MOEX ISS REST SSL=True → MOEX ISS REST SSL=False), программа падает с `RuntimeError` без graceful exit. Макро/дивиденды/фундаментал tolerated с warning + sentinel, а OHLCV критичен — без свечей вся модель неработоспособна. Решение: при каждом успешном прогоне сохранять `outputs/stock_modelv16/cache/last_known_ohlcv_{ticker}.csv` (или один общий parquet); при провале всех онлайн-источников читать оттуда с явным warning о сроке давности (`last_modified` vs `today`). Если данные старше N дней (например, 7) — fail fast с понятным сообщением, не молча использовать устаревшее. Реализация: ~30 строк в `load_stock_data_moex_test`. Польза особенно для multi-date backtest и demo во время защиты — выход из строя сети MOEX не убивает прогон.
...
**Online learning при live-deployment (часть этапа 3)**

Текущая модель — batch learning: один прогон обучает walk-forward → предсказывает → завершается. В live-режиме paper-trading возникает проблема **деградации при смене рыночного режима** (distribution shift / concept drift): модель, обученная в марте, через 2 месяца может потерять качество из-за изменения макрофона, новых новостей, смены тренда.

Варианты решения (от простого к сложному):

| Подход | Реализация | Сложность |
| --- | --- | --- |
| **Daily cron retraining** | APScheduler / Windows Task Scheduler в 22:00 запускает полный walk-forward на расширенных данных | низкая (~2 ч) |
| **Incremental XGBoost** | `xgb.train(..., xgb_model=existing)` дообучает деревья на свежих данных без полного цикла | средняя (~4 ч), риск deg |
| **LSTM fine-tuning** | Продолжение обучения с малым lr (1e-5) на последних 30 днях | средняя, риск catastrophic forgetting |
| **Sliding window training** | Учиться только на последних N днях, забывать старое автоматически | низкая (~3 ч) |
| **Concept drift detection** | ADWIN/DDM/Page-Hinkley на скользящем MAE → триггер полного переобучения при ухудшении | высокая (~10 ч) |

Рекомендуемый старт: **daily cron retraining**. Простейший и достаточный вариант — каждый вечер после закрытия торгов полностью переобучить модель. На утро есть свежая модель, не требует менять архитектуру. Минус: 20–30 минут GPU compute каждый день, но это приемлемо для одного исследователя.

Concept drift detection — следующая итерация, если daily retraining окажется недостаточно реактивным (например, при резкой смене ставки ЦБ модель деградирует за часы, а не за дни).

**Защитимая формулировка для комиссии** (если спросят про online learning на защите): *"Текущая работа — batch learning с walk-forward валидацией, что соответствует академической постановке. Online learning имеет смысл только в условиях live deployment, запланированного как этап 3 после защиты. На этом этапе будет реализовано периодическое переобучение (daily cron) как baseline и detection of distribution shift как развитие. Сейчас это вне scope работы."*
