...
- Метрики: абсолютная ошибка (RUB), Error %, **Naïve baseline** (ошибка тривиального прогноза «цена не изменится»), **R² на доходностях** (честная мера качества, не зависящая от трендовости ряда), **IC (Spearman)** (ранговая корреляция предсказанных и реальных доходностей, стандарт quant-индустрии), CI Coverage, Direction Accuracy
