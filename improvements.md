...
- **TCSG данные обрываются 2024-11-27** — TCS Group переименован/реструктурирован в **«Т-Технологии»** (тикер MOEX: **T**) в конце 2024. MOEX просто прекратил торги по тикеру TCSG. На сегодня (2026-05) модель учится на 5-летней истории, заканчивающейся 17 месяцев назад. R² катастрофический (h=1=0.014, h=2=−0.31, h=3=−1.00), потому что walk-forward OOS-окна — это последние 5% данных = сентябрь–ноябрь 2024. Решение: добавить автосклейку `TCSG → T` (по аналогии с YNDX→YDEX) в `_load_single_ticker`, или явно обновить тикер в дефолтном списке бэктеста.
- **YDEX: структурный разрыв в июле 2024 портит R²** — автосклейка YNDX (Yandex N.V., Нидерланды) + YDEX (МКПАО Яндекс, РФ) даёт длинную историю (2014→2026, 3024 точек), но это **формально две разные компании** с разным составом акционеров и драйверами цены. R²=0.493/0.403/0.320 — модель видит структурный шок и не знает что с ним делать. MAPE при этом ~4% (адекватно), просто R² плохо отражает качество на смешанной серии. Опции: (a) учить модель только на YDEX (с 2024-07, ~500 точек — слишком мало); (b) учить только на YNDX (до 2024-07, замороженная история — устарела); (c) добавить дамми-признак `is_post_restructure` и оставить склейку — модель сама поймёт сдвиг режима.
