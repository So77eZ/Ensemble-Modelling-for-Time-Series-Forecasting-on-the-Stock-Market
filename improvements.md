...
- **Условный пропуск LSTM на длинных горизонтах** — после внедрения `Ridge(positive=True)` (v15.10) обнаружилось: на SBER LSTM полезен только на h=1 (вес 0.350), на h=2/h=3 Ridge сам обнуляет вес LSTM (0.000), модель сводится к чистому XGB. Сейчас LSTM-ансамбль (3 seeds × 3 folds = 9 моделей на горизонт) обучается всё равно — пустая трата времени для горизонтов, где он будет отброшен. **Идея**: после Optuna для h=1 проверять `LSTM_coef`, и если он < threshold (например 0.05) — на следующих горизонтах не обучать LSTM, использовать только XGB. Потенциальное ускорение: ~2× для horizons где LSTM бесполезен (для SBER это h=2 и h=3 → ~33% от общего времени тренировки). Риск: для разных тикеров поведение разное — нужно проверить на бэктестах LKOH/GAZP/TCSG/YDEX перед внедрением.
