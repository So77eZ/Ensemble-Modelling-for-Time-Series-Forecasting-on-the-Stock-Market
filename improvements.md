...
- **Фундаментальные данные загружаются повторно** — T-Bank API вызывается отдельно для каждого горизонта (3 раза); нужно загружать один раз и передавать как параметр (аналогично `macro_data`).
