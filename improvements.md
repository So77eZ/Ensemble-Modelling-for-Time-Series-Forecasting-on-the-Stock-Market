...
- **Условный пропуск LSTM (или его удаление)** — после внедрения `Ridge(positive=True)` (v15.10) проведён замер по 5 тикерам × 3 горизонта = 15 случаев. **Результат**: только 1 случай из 15 имеет значимый вес LSTM (SBER h=1 = 0.350). LKOH h=1 = 0.023 (пренебрежимо). Остальные **13 случаев** — LSTM = 0.000.

  | Тикер | h=1 | h=2 | h=3 |
  | --- | --- | --- | --- |
  | SBER | **0.350** | 0.000 | 0.000 |
  | LKOH | 0.023 | 0.000 | 0.000 |
  | GAZP | 0.000 | 0.000 | 0.000 |
  | TCSG | 0.000 | 0.000 | 0.000 |
  | YDEX | 0.000 | 0.000 | 0.000 |

  Вывод: XGBoost с расширенным набором фичей (макро + дивы + технические + сезонные) **доминирует**. LSTM-ансамбль (3 seeds × 3 folds = 9 моделей на горизонт) — пустая трата ~30% времени тренировки на 14 из 15 случаев.

  **Три опции** (по возрастанию радикальности):
  - **A. Кэш «бесполезности» LSTM** — после Optuna сохранять флаг `lstm_useful_for_<ticker>_h<N>`; на следующих запусках того же тикера/горизонта пропускать тренировку LSTM. Безопасно, обратимо, ускорение 14/15 ≈ 93%.
  - **B. Удалить LSTM из пайплайна** — модель сводится к XGB + Ridge с 1 фичей = просто XGB. Ускорение ~3-4×, упрощение кода. Потеря: SBER h=1 (R² потенциально упадёт на 1-3 п.п.). Подходит, если для диплома важен прагматизм, не охват архитектур.
  - **C. Починить LSTM** — текущая архитектура/гиперпараметры могут быть плохо настроены. Попробовать: глубже сеть, attention, другие фичи. Большая работа, неясный исход.
